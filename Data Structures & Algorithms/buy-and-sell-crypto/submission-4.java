class Solution {
    public int maxProfit(int[] prices) {
        ...
        // solução 2: 9 min - 21 min - travei
        /*
        int left = 0, right = prices.length-1;
        while (left < right) {
            int buyPrice = prices[left]; // 5, 5
            int sellPrice = prices[right];// 10,
            int balance = sellPrice - buyPrice; // 5
            maxProfit = Math.max(maxProfit, balance); // 5
        ...
        // solução 3 - li as 2 primeiras dicas, vou tentar hashmap
        /*
        int buyPrice = 0;
        int sellPrice = 0;
        Map<Integer, List<Integer>> profitableTransactions = new HashMap<>();
        for (int price : prices) {
            if (profitableTransactions.contains(price)) {
                continue;
            }
            
        }*/
        // solução 4: iniciada aos 30 min
        // passou por mais test cases que a solução 2
        int left = 0, right = 1;
        ...
        while (left < right && right < prices.length) {
            ...
            int balance = sellPrice - buyPrice; // 5
            maxProfit = Math.max(maxProfit, balance); // 5
            System.out.printf("buyPrice[%d]: %d | sellPrice[%d]: %d | min: %d | max: %d%n", left, buyPrice, right, sellPrice, min, max);
            if (buyPrice > sellPrice) {
                if (sellPrice < min) // resolvido aos 42 min, 2 hints
                    left = right;
                else
                    left++;
            }
            min = Math.min(buyPrice, min);
            max = Math.max(buyPrice, max);
            right++;
        }*/
        // claude
        for (int price : prices) {
