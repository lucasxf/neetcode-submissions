...
You must write an algorithm that runs in O(n) time.
        */
        ...
        for (int i = 0; i < nums.length; i++) {
            int n = nums[i] + 1;
            System.out.println("n: " + n + " | i: " + i + "| curr: " + curr + " | result: " + result);
            while (uniqueNums.contains(n)) {
                // talvez eu devesse remover os itens já vistos do set
                // para otimizar
                System.out.printf("n: %d | i: %d | nums[%d]: %d | curr: %d | result: %d%n", n, i, i, nums[i], curr, result);
                /*if (curr == 0) {
                    curr = Math.max(result, curr);
                }*/
                curr++;
                n++;
                System.out.printf("n: %d | i: %d | nums[%d]: %d | curr: %d | result: %d%n%n", n, i, i, nums[i], curr, result);
            }
